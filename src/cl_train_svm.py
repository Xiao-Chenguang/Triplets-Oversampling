...
def run(
    ...
    **kwargs,
    ...
    if dsname in {"mnist", "cifar10", "cifar100", "fashionmnist", "svhn"}:
        ...
        x_train = x_train.float().flatten(start_dim=1) / 255.0
        x_test = x_test.float().flatten(start_dim=1) / 255.0
    ...
    log_header = (
        f"{dsname},{ir},{sampling},{m_neighbors},{n_neighbors}{str_values},{seed}"
    )
    ...
    print(
        "acc: {:.3f}, recall: {:.3f}, precision: {:.3f}, f1: {:.3f}, gmean: {:.3f}, auc: {:.3f}, ap: {:.3f}".format(
            *score
        )
    )
