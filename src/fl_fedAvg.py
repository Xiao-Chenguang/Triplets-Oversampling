...
from .fl_tools import add_state, compute_updat, weight_sum
...
class Federation:
    ...
    def global_step(self):
        ...
        selected_clients = torch.randperm(self.args.num_clients)[
            : self.args.clients_per_round
        ]
        ...
        self.global_round += 1
        self.logger.info(
            f"Global round {self.global_round} finished. ({time.time()-start:.2f})s"
        )
    ...
    def train(self):
        ...
        self.logger.info(
            "Start training Fed learners with follow settings:\n"
            + "\n".join(
                [f"{k:>17} = {vars(self.args)[k]}" for k in sorted(vars(self.args))]
            )
            + "\n"
            + "=" * 80
            + "\n"
    ...
    def __tb_writer__(self, args):
        ...
        with open(path, "w") as f:
            f.write(
                "dataset,ir,sampling,seed,epoch,acc,recall,precision,f1,gmean,auc,ap\n"
            )
...
class FedAvg(Federation):
    ...
    def client_fit(self, selected_clients):
        ...
        for i, client in enumerate(selected_clients):
            ...
            update, client_loss = train_client(
                self.global_model, self.dataloaders[client], self.args, self.logger
            )
            updates.append(update)
            loss += client_loss
        loss /= len(selected_clients)
        print(
            f"Epoche [{self.global_round+1:4d}/{self.args.global_epochs}] >> train loss: {loss:.5f}"
        )
        return updates
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for x, y in self.test_loader:
                ...
                loss += batch_loss
                self.logger.debug(
                    f"Test a batch of {y.shape[0]} samples. ({time.time()-batch_start:.2f})s"
                )
        self.logger.info(
            f"Finish test on {test_size} samples. ({time.time()-start:.2f})s"
        )
        loss /= test_size
        ...
        with open(self.writer[0], "a") as f:
            f.write(
                "{},{},{},{},{},{},{},{},{}\n".format(
                    self.writer[1], self.global_round, *score
                )
            )
...
def train_client(global_model, dataloader, args, logger):
    ...
    optimizer = optim.SGD(
        local_model.parameters(), lr=args.local_lr, momentum=args.local_momentum
    )
    lossfn = nn.BCEWithLogitsLoss()
